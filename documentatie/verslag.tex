...
Om snel inzicht te geven in de implementatie, volgen hier twee korte en representatieve fragmenten uit \texttt{src/parser.pl} (DCG-regels):
...
De uitvoering van een programma start in \texttt{run\_file/2}. Na het parsen van het pwat-bestand wordt de
bekomen module doorgegeven aan \texttt{run\_module/2}. Daar worden eerst de data-segmenten omgezet naar een
geheugenvoorstelling via \texttt{build\_memory/2}. Vervolgens wordt de startfunctie van de module uitgevoerd.
...
Dit predicaat geeft de grote lijn van de uitvoering in drie stappen: eerst wordt de functie klaargezet
(argumenten en locals), daarna worden de instructies uitgevoerd, en op het einde wordt het eindsignaal
verwerkt om de status en returnwaarden te bepalen.

Wat ook belangrijk is is het signaal-mechanisme: instructies geven signalen door zoals
\texttt{continue}, \texttt{returned}, \texttt{trap}, \texttt{invalid} of \texttt{break(N)}.
Die signalen bepalen of de uitvoering doorgaat of stopt. Verderop in deze sectie bespreken we dat stap voor stap.

Ook de stack-werking volgt hetzelfde idee doorheen de hele interpreter: elke functie start met een lege
operand-stack, waarden worden tijdens de uitvoering gepusht en gepopt, en bij een \texttt{call} gaan argumenten
van caller naar callee en komen returnwaarden terug naar de caller.
...
\begin{itemize}
    ...
    \item \texttt{invalid} : ongeldig programma (bv. stack-underflow of ongeldige local)
...
Indien een instructie onvoldoende waarden op de stack vindt of een ongeldige local aanspreekt, wordt een \texttt{invalid} gegenereerd.
...
De \texttt{analyse} en \texttt{paths} modi gebruiken dezelfde interpreter als de \texttt{run}-modus, maar met een context die extra informatie bijhoudt.
Die context wordt in \texttt{src/main.pl} aangemaakt:
\begin{lstlisting}
ContextIn = analyse(MaxInstructions, 0, [], []).
\end{lstlisting}

De term heeft de vorm \texttt{analyse(MaxInstructions, CurrentCounter, Inputs, PathTrace)}. Tijdens de uitvoering worden velden bijgewerkt door \texttt{src/function\_runner.pl}:

\begin{itemize}
    \item \texttt{increment\_instruction\_counter/2} verhoogt \texttt{CurrentCounter} na elke instructie.
    \item \texttt{record\_branch\_decision/3} voegt beslissingen toe aan \texttt{PathTrace}, zodat we kunnen zien welke takken zijn genomen.
\end{itemize}

Belangrijk: in analyse-modus worden sommige primitieve functies (bijv. \texttt{read\_int} en \texttt{rand\_int}) als bronnen van meerdere mogelijke waarden behandeld. In plaats van één vaste uitkomst enumereren ze in de analyse meerdere waarden, wat resulteert in meerdere mogelijke uitvoeringspaden. De combinatie van deze niet-deterministische primitieve keuzes en de vastgelegde beslissingen in \texttt{PathTrace} maakt padverkenning mogelijk.

Om alle mogelijke paden te verzamelen roept het programma herhaaldelijk de analyse aan en gebruikt \texttt{findall} (via \texttt{collect\_all\_results/3}) om alle \texttt{Status-Inputs-PathTrace} combinaties te verzamelen. Daarna worden deze resultaten gefilterd en gededupliceerd (bijv. met \texttt{prefer\_complete\_trap\_paths/2}, \texttt{prefer\_decision\_paths/2} en \texttt{keep\_first\_input\_per\_path/2}) zodat de output per pad overzichtelijk blijft.

De limiet \texttt{MaxInstructions} voorkomt dat oneindige paden te lang doorgaan: zodra \texttt{CurrentCounter} \textgreater= \texttt{MaxInstructions} stopt die run.
...
\begin{itemize}
    ...
    \item \textbf{Exit-code-tests}: controleren of het programma de juiste exit codes retourneert voor verschillende scenario's. Dit omvat: exit code 0 bij succesvolle uitvoering, exit code 1 voor ongeldig programma's en parsing-fouten, en exit code 2 wanneer een trap optreedt.
\end{itemize}
