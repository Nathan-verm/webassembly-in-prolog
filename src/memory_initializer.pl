...
build_memory_helper([data(Address, RawStr)|Rest], Acc, Memory) :-
    ...
    build_memory_helper(Rest, [adress(Address, DecodedStr)|Acc], Memory). % there is a typo here lol
...
% keep taking until a non-digit result, split list 
% Digits = ['1','2','3']
% Tail   = ['\\','A']

take_digit_chars([C|Rest], [C|Digits], Tail) :- % take_digit_chars(['1', '2', '3', '\\'], ['1', '2', '3'], ['\\'])
    char_type(C, digit),
    !, % needed because if it is a digit we don't want backtracking 
